...
pte_t* kpte;
if(newsize>=PLIC)
  panic("kvmmapuser: new size too large");
for(va=oldsize;va<newsize;va+=PGSIZE)
  {
  upte=walk(user_p,va,0);
  if(upte==0)
    {printf("kvmmapuser:0x %x  0x %x",va,newsize);
    panic("kvmmapuser: no upte");
    }
  if((*upte&PTE_V)==0)
    {
      printf("kvmmapuser:no valid pte 0x %x  0x %x",va,newsize);
      panic("kvmmapuser: no valid upte");
    }
  kpte=walk(kernel_p,va,1);
  if(kpte==0)
...
//print the virtualmemory page
void vmprint_s(pagetable_t pagetable, int n)
{
  if (n == 3)
    return;
  for (int i = 0; i < 512; i++)
  {
    ...
    if (pte & PTE_V)
    {
      ...
      for (int k = 0; k < n; k++)
        printf(".. ");
      printf("..%d: pte %p pa %p\n", i, pte, child);
      vmprint_s((pagetable_t)child, n + 1);
    }
  }
}
void vmprint(pagetable_t pagetable)
{ printf("page table %p\n",pagetable);
  vmprint_s(pagetable, 0);
}
